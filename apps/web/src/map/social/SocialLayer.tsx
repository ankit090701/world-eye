...
import { socialMapStore, useSocialMap } from '../../data/socialStore'
...
export function SocialEngine() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const r = await fetchSocialMap()
        if (!cancelled) socialMapStore.update(r.points, r.origin)
      } catch {
        /* keep last */
      }
    }
...
export function SocialSync() {
  ...
  const snap = useSocialMap()

  useEffect(() => {
    if (!map || epoch === 0) return
    const features: Feature[] = snap.points.map((p) => ({
      type: 'Feature',
      properties: { place: p.place, count: p.count, source: p.source, title: p.title, url: p.url },
      geometry: { type: 'Point', coordinates: [p.lon, p.lat] },
    }))
    setSocialData(map, { type: 'FeatureCollection', features })
  }, [map, epoch, snap])
...
export function SocialInteractions() {
  ...
  useEffect(() => {
    ...
    map.on('click', LYR.socialPoints, onClick)
    map.on('mouseenter', LYR.socialPoints, onEnter)
    map.on('mouseleave', LYR.socialPoints, onLeave)
    return () => {
      map.off('click', LYR.socialPoints, onClick)
      map.off('mouseenter', LYR.socialPoints, onEnter)
      map.off('mouseleave', LYR.socialPoints, onLeave)
      popup.remove()
    }
  }, [map, activeTool])
